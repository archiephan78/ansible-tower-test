...
fio_test() {
	if [ -e '/usr/bin/fio' ]; then
		...
		#io --randrepeat=1 --ioengine=libaio --direct=1 --gtod_reduce=1 --name=fio_test --filename=fio_test --bs=4k --numjobs=1 --iodepth=64 --size=256M --rw=randwrite --runtime=300 --time_based --output="$tmp"
